...
function get_country_passport($entity_id, $name, $code, $bonChange=false, $bonClick=false, $bonDisable=false,$bonVisible=false,$width = 40) {
	...
	if ($bonClick)
	 ...
	 $db = opendatabase();
	 $sql = "SELECT * FROM TDWE_ENTITY WHERE ENT_ID = '$entity_id' order by ENT_DESC1 ASC";
	 $rs = mysql_query($sql);
...
function get_entity_combo($entity_id, $name, $code, $bonChange=false, $bonClick=false, $bonDisable=false,$bonVisible=false,$width = 40) {
	...
	if ($bonClick)
	 ...
	 $str = "<select required name='$name' class='form-control'>
...
function get_entity_combo2($entity_id, $name, $code, $bonChange=false, $bonClick=false, $bonDisable=false,$bonVisible=false,$width = 40) {
	...
	if ($bonClick)
	 ...
	 $db = opendatabase();
	 $sql = "SELECT * FROM TDW_ENTITY WHERE ENT_ID = '$entity_id' AND ENT_CODE  NOT IN ('89','90','98') order by ENT_ID ASC";
	 $rs = mysqli_query($db,$sql);
	 $str = "<select required name='$name' class='form-control'>
	 			<option value=''>--Sila Pilih--</option>";
	 while ($row=mysqli_fetch_assoc($rs)) {
...
function get_gender_desc($entity_id, $name, $code){
	$code = intval($code);
